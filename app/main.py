...
# Подавление системных предупреждений PyTorch/DINOv2 (xFormers/SwiGLU)
...
# Пути к весам детекции и модерации
...
if MODERATOR_WEIGHTS.exists():
    ...
    print(f"❌ CRITICAL ERROR: Moderator weights NOT FOUND at {MODERATOR_WEIGHTS}")
...
# --- Блок 3: Собачья биометрия (YOLO + DINOv2 ViT-S/14 384D) ---
dog_service = None
try:
...
# --- Блок 4: Кошачья биометрия (YOLO + DINOv2 ViT-S/14 384D) ---
cat_service = None
try:
    from app.services.cat_biometrics_service import CatBiometricsService

    cat_service = CatBiometricsService(
...
app.include_router(
    biometrics_router, prefix="/api/v1", tags=["Biometrics"]
)

try:
    from app.api.v1.endpoints.cat_biometrics import router as cat_biometrics_router
    app.include_router(
        cat_biometrics_router, prefix="/api/v1", tags=["Cat Biometrics"]
    )
    print("✅ Cat Biometrics Router: Mounted at /api/v1")
except ImportError:
    pass
